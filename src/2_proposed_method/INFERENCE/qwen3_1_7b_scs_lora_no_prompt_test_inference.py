# Inference-only: qwen3_1_7b_scs_lora_no_prompt (test)
...
MODEL_FAMILY = 'qwen'
VARIANT_NAME = 'qwen3_1_7b_scs_lora_no_prompt'
ADAPTER_FILENAME = 'qwen3_1_7b_scs_lora_no_prompt_best_adapter.pt'
TRAINING_PROMPT_MODE = 'question_context_only'
CASCADED_INPUT_SCALE = 1.0
...
def build_input(question: str, context: str) -> str:
    # EXACT SCS no-prompt training format.
    # No question-type label and no type-specific instruction in generator text.
    return (
...
def load_scs_adapter(model: nn.Module, path: Path) -> Dict:
    ...
    expected_prompt_meta = 'question_context_only'
    expected = {
...
def generate_batch(model, tokenizer, router, retriever, rows: pd.DataFrame, device: str):
    ...
    # Router probabilities affect ONLY expert routing. They are NOT inserted into text.
    prompts = [build_input(q, context_used) + PROMPT_SUFFIX for q, context_used in zip(questions, contexts_used)]
    inputs = tokenizer(

# Inference-only: qwen3_1_7b_question_type_mixture_lora — SOFT ROUTING (test)
# RAG: BGE-M3 dense + BM25 + RRF + original BGE reranker.
# Question-Type Mixture-of-LoRA: 4 experts map to COMPARISON/FACTOID/SUMMARY/VERIFICATION.
# Training used gold one-hot question_type routing; inference uses PhoBERT predicted FULL probability soft routing.
# Predicted argmax question type is used only to construct the same type-aware prompt as training.
# test means the standard test.csv split.
...
MODEL_FAMILY = 'qwen'
VARIANT_NAME = 'qwen3_1_7b_question_type_mixture_lora'
ROUTING_MODE = 'soft'
OUTPUT_VARIANT_NAME = f'{VARIANT_NAME}_soft_routing'
ADAPTER_FILENAME = 'qwen3_1_7b_question_type_mixture_lora_best_adapter.pt'
...
OUTPUT_DIR = OUTPUT_BASE_DIR / OUTPUT_VARIANT_NAME / INFERENCE_SPLIT
...
PRED_FINAL_PATH = OUTPUT_DIR / f'{OUTPUT_VARIANT_NAME}_{INFERENCE_SPLIT}_predictions.csv'
if NUM_SHARDS == 1:
...
# Standard-test inference uses test.csv.
...
print('ROUTING_MODE:', ROUTING_MODE)
...
def build_input(question: str, context: str, predicted_type: str) -> str:
    # Match the Question-Type Mixture-of-LoRA type-aware training text, using predicted type at inference.
...
# %% [cell 6]
# ============================================================
# 4. PHOBERT SOFT ROUTER — SAME ROUTING SIGNAL AS SCS-LoRA
# ============================================================
...
# %% [cell 7]
# ============================================================
# 5. QUESTION-TYPE MIXTURE-OF-LORA INFERENCE ARCHITECTURE
#    4 experts:
#    0=COMPARISON, 1=FACTOID, 2=SUMMARY, 3=VERIFICATION
#
#    Inference routing uses the same PhoBERT full probability vector
#    as the SCS-LoRA main inference, so the only generator-side
#    architectural difference is:
#
#      QT-MoLoRA: base(x) + sum_i p_i * expert_i(x)
#      SCS-LoRA : base(x) + shared_out(x)
#                         + sum_i p_i * expert_i(x + cascade(x))
# ============================================================
class LoRAExpert(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_adapter = self.dropout(
            x.to(dtype=adapter_dtype)
        )
        out = (
            self.lora_B(
                self.lora_A(x_adapter)
            )
            * self.scaling
        )
...
class QuestionTypeMixtureLoRALinear(nn.Module):
    ...
    def set_inference_route_weights(
        self,
        weights: Optional[torch.Tensor],
    ):
        self.inference_route_weights = weights

    def _mix_soft_experts(
        self,
        x: torch.Tensor,
        alpha: torch.Tensor,
    ) -> torch.Tensor:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.inference_route_weights is None:
            raise RuntimeError(
                'Question-Type Mixture-of-LoRA inference '
                'route weights are unset.'
            )

        base_out = self.base_layer(x)
        type_out = self._mix_soft_experts(
            x,
            self.inference_route_weights,
        )

        # No shared_out and no shared_input/cascade branch.
        return base_out + type_out
...
def inject_question_type_mixture_lora(
    model: nn.Module,
) -> nn.Module:
    replaced = []
    ...
    print(
        'Injected Question-Type Mixture-of-LoRA modules:',
        len(replaced),
    )

    if len(replaced) != EXPECTED_INJECTED_MODULES:
        raise RuntimeError(
            f'Expected {EXPECTED_INJECTED_MODULES} q/k/v/o modules '
            f'for {BACKBONE_MODEL}, but injected {len(replaced)}.'
        )

    return model


def set_qt_mixture_route_weights(
    model: nn.Module,
    weights: Optional[torch.Tensor],
):
    for module in model.modules():
        if isinstance(
            ...
            module.set_inference_route_weights(
                weights
            )
...
def load_qt_mixture_adapter(
    model: nn.Module,
    path: Path,
) -> Dict:
    payload = _load_torch_payload(
        path
    )
    ...
    if (
        ...
        raise ValueError(
            f'Invalid Question-Type Mixture-of-LoRA '
            f'adapter payload: {path}'
        )

    expected_format = (
        'question_type_mixture_lora_adapter_v1'
    )
    ...
    expected = {
        ...
        'num_experts':
            NUM_EXPERTS,
        'shared_output_adapter':
            False,
        'shared_input_adapter':
            False,
        'cascaded_input':
            False,
    }
    ...
    for key, value in expected.items():
        if key not in meta:
            raise ValueError(
                f'Adapter metadata missing '
                f'required field {key!r}'
            )
        if meta[key] != value:
            raise ValueError(
                f'Adapter metadata mismatch {key}: '
                f'expected {value!r}, '
                f'got {meta[key]!r}'
            )
    ...
    if unexpected_state_names:
        raise RuntimeError(
            'Question-Type Mixture-of-LoRA adapter '
            'contains non-expert tensors: '
            f'{unexpected_state_names[:20]}'
        )
    ...
    if (
        ...
        in meta
        and int(
            meta['adapter_parameter_count']
        ) != state_numel
    ):
        raise RuntimeError(
            'Adapter parameter count mismatch: '
            f'meta={meta["adapter_parameter_count"]}, '
            f'state={state_numel}'
        )
    ...
    if unexpected:
        raise RuntimeError(
            'Unexpected Question-Type Mixture-of-LoRA '
            f'keys: {unexpected[:20]}'
        )

    print(
        'Loaded complete Question-Type Mixture-of-LoRA adapter:',
        path,
    )
    print(
        'Adapter tensors:',
    ...
    print(
        'Missing base-model keys (expected):',
        len(missing),
    )
...
# %% [cell 10]
# ============================================================
# 8. MODEL BUILD — BF16-SAVED QT-MoLoRA ADAPTER -> FP16 INFERENCE
# ============================================================
...
def build_inference_model(device: str):
    ...
    for p in model.parameters():
        p.requires_grad = False
    model = inject_question_type_mixture_lora(model)
    meta = load_qt_mixture_adapter(model, adapter_path)
    ...
    print('Question-Type Mixture-of-LoRA inference adapter dtypes:', dtypes)
...
def generate_batch(model, tokenizer, router, retriever, rows: pd.DataFrame, device: str):
    ...
    # SOFT ROUTING: use the complete PhoBERT probability vector.
    ...
    set_qt_mixture_route_weights(model, route_weights)
    try:
        ...
        set_qt_mixture_route_weights(model, None)
...
def predict_streaming(model, tokenizer, router, retriever, device: str):
    ...
    bar = tqdm(total=len(pending), desc=f'{OUTPUT_VARIANT_NAME} {INFERENCE_SPLIT}')
